...
const Task = {
  createTask: async (userId, title) => {
    try {
      ...
      // Corrected Logging: Pass the error object as a separate argument
      console.error(
        `ERROR: Failed to create task for user_id: ${userId}, title: ${title}.`,
        error // <-- Pass the error object here
      );
      throw {
        status: "error",
        message: "Failed to create task due to a database error.",
        error: error.message, // Still send only the message to the client
      };
    }
  },
  ...
  getTasksByUser: async (userId) => {
    try {
      ...
      // Corrected Logging
      console.error(
        `ERROR: Failed to retrieve tasks for user_id: ${userId}.`,
        error
      );
      throw {
        status: "error",
  ...
  updateTask: async (taskId, title, userId) => {
    try {
      ...
      // Corrected Logging
      console.error(
        `ERROR: Failed to update task task_id: ${taskId}, user_id: ${userId}.`,
        error
      );
      throw {
        status: "error",
  ...
  getAllTasks: async () => {
    try {
      ...
      // Corrected Logging
      console.error(`ERROR: Failed to retrieve all tasks.`, error);
      throw {
        status: "error",
  ...
  deleteTask: async (taskId, userId) => {
    try {
      ...
      // Corrected Logging
      console.error(
        `ERROR: Failed to delete task task_id: ${taskId}, user_id: ${userId}.`,
        error
      );
      throw {
        status: "error",
